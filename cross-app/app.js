...
/**
 * 跨源 App 外壳下，产物链接也要指向宿主而非 App 自己的 origin。
 * 同源部署时 index.html 控制器不会设置 window.__AGENT_UI_API__，这里退化为原路径。
 */
function apiPathForApp(path) {
  return typeof window !== "undefined" && typeof window.__AGENT_UI_API__ === "function"
    ? window.__AGENT_UI_API__(path)
    : path;
}
...
function patchArtifacts(parts, files, runId, callbacks) {
  ...
  host.innerHTML =
    ...
    files
      .map((f) => {
        const href = apiPathForApp(`/api/runs/${encodeURIComponent(runId)}/artifact?path=${encodeURIComponent(f.path)}`);
