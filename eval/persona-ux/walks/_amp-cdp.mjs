...
const PROFILE = resolveChromeUserDataDir({ name: "amp-walk" });
const OUT = "d:\\Work\\Github_pros\\Agent_Design\\eval\\persona-ux\\walks";
const STATE = `${OUT}/_amp-cdp-state.json`;
const URL = "http://127.0.0.1:4173/#walk=amp";
...
async function connectPage() {
  ...
  const page =
    targets.find((t) => t.type === "page" && String(t.url).includes("walk=amp")) ||
    targets.find((t) => t.type === "page" && String(t.url).includes("4173")) ||
...
if (cmd === "launch") {
  ...
  if (!String(page.url).includes("walk=amp")) {
...
if (cmd === "shot") {
  const name = process.argv[3] || `_amp-${Date.now()}.png`;
  const { ws, send } = await connectPage();
