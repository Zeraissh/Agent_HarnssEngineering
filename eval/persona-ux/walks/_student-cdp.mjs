...
const PROFILE = resolveChromeUserDataDir({ name: "student-walk" });
const OUT = "d:\\Work\\Github_pros\\Agent_Design\\eval\\persona-ux\\walks";
const STATE = `${OUT}/_student-cdp-state.json`;
const URL = "http://127.0.0.1:4173/#walk=student";
...
async function connectPage() {
  ...
  const page =
    targets.find((t) => t.type === "page" && t.url.includes("walk=student")) ||
...
if (cmd === "launch") {
  ...
  if (!page.url.includes("walk=student")) {
...
if (cmd === "shot") {
  const name = process.argv[3] || `_student-${Date.now()}.png`;
  const { ws, send } = await connectPage();
