...
对照 [`remediation-v2.md`](remediation-v2.md) 的问题编号（V-01~V-34）与验收标准
（AC2-01~AC2-18），逐条记录关闭状态与证据。

**状态图例**：⬜ 未开始 · ✅ 关闭（附第一手证据）· ⚠️ 部分关闭（附缺口）

> **§1.5 重排（2026-08-08）**。这张表长到 52 行之后开始自己骗自己，三类问题一并修掉：
>
> 1. **表头范围写错**：写的是 V-01~V-21，表体实际有 **34 条 V**；
> 2. **写死的数字全过期**：基线那行的「225 测试 / 16 文件」、下方规模表停在 R11=485，
>    实际是 **641 测试 / 21 文件**。**规模表已删**——逐轮追加的增量表是这类漂移的
>    固定产地，而"现在多少条"跑一次 `npx vitest run` 就有，不需要文档替它记账；
> 3. **证据引用已删除的东西**：R-03 的"三标签"、V-28 的"追加输入框"、AC-05 的
>    "10 个交互元素 / `#0969da`"都指向不存在的结构。这类行已就地标注，
>    **不删证据、只标明它描述的是哪一代形态**——删掉就等于抹掉当时确实验过的事实。
>
> 另一条纪律：**同一件事不许在两张表里各记一遍**。四视口零溢出此前被 R-02 / AC-08 /
> AC2-15 记了三遍且三遍都没有自动化；屏幕阅读器被 AC-06 / AC2-16 记了两遍。
> 重复项现在统一指向 AC2-18 那份真机清单。

基线 HEAD：`632fdb8`。**当前规模**：641 测试 / 21 文件（`npx vitest run` 实测，
2026-08-08）——这个数字**不要抄进别处**，抄一次就多一个会漂的副本。
...
| AC2-11 1000 事件单帧 < 16ms | ✅（2026-08-08 真机量化） | `createBatcher` 测试断言 1000 条事件只触发 1 次 flush（锁死 O(n²) 不回归）；日志侧 1000 行渲染后再来一条只新增 1 个节点、首节点仍是原对象。<br>**真机量化（4173，Chrome）**：量出来的结论与预期相反——**脚本不是瓶颈**：2000 条时 `reduceEvents` 0.8ms、`appendOnly` 全量走一遍 0.3ms、`deriveLogEntries` 0.8ms；把 activeTab 换成不含日志的 Context 面，同样 2000 条下单帧从 24.4ms 掉到 **0.9ms**——钱全花在**一棵 1.3 万节点日志树的布局**上。<br>改前每帧随条目数线性长：2.5@250 / 5@500 / 9.5@1000 / 24@2000（**约 1400 条掉出预算**）。加一行 `content-visibility: auto` 之后实测 **2.1@500 / 3.6@1000 / 6.1@2000 / 14.1@4000**，预算线推到 ~4200 条。已确认跳过渲染的条目**仍在可访问性树里**、仍能被页内查找命中、`scrollIntoView` 照常工作。回归锁：`ui-app.test.ts`「AC2-11 长日志的单帧预算靠 content-visibility 守住」|
...
| AC2-15 四视口 × 双主题无溢出 | ✅（形态已变，2026-08-08 重验） | ⚠️ 下面这次 32 组测的是**顶栏提交栏**那一代页面；此后提交栏移到底部、新增 `#action-dock`、追加框并入底栏。**已按新形态重测四视口零溢出**（见 AC2-18 行）。原始记录：**32 种组合零溢出**（390/768/1280/1440 × light/dark × Loop/Context/Tools/Verification），逐格实测 `scrollWidth - clientWidth`。窄屏另确认侧栏 `display:none` 与返回按钮在位（R-02 原始要求）。200% 缩放零横向溢出、标签与主操作齐全（AC-09 未回退）|
| AC2-16 axe 六画面双主题零 violations | ✅ | 画面集合随新 IA 扩为八个（空态 / 列表 / Loop / Context / Tools / Verification / 窄屏 / 宿主快照缺席降级），**八个画面 violations 恒空**。⚠️ **口径更正（2026-08-08 复验）**：此前记作"两套主题各扫一遍共 16 次"是**高估**——渲染层全文不读 `data-theme`，且 jsdom 从不载入 `styles.css`，所以 dark 那 8 次是 light 那 8 次的逐字重跑，`snapshot(dark) == snapshot(light)` 结构上不可能不等。实际强度是**8 个画面的结构语义**；与主题真正相关的可访问性由 `ui-app.test.ts` 的对比度门禁（解析 CSS 实算 WCAG）承担，两者互补而不是 2× |
| AC2-17 npm run ui 可启动且绑本地 | ✅ | `ui/serve.ts` 落地，默认 `127.0.0.1:4173`；纳入 tsconfig 后受 `npm run typecheck` 覆盖 |
| AC2-18 v1.0 全部条目未回退 | ⚠️ 复验已做，抓到 4 处实缺陷（已修），2 处只能人工 | **2026-08-08 逐条复验**。做法：对每条去找"能证明它没回退"的第一手证据，并对可疑证据做**变异测试**（把被守护的东西改坏，看有没有测试变红）。结果 40 条里 18 条已被自动化锁死、13 条需真机、9 条因 v2 结构变更而作废（被取代物守住，逐条注明）。<br>**抓到 4 处真缺陷，全部已修**：①AC-04 展开日志第一次点击是死点击（宿主自写 toggle，被测的 `toggleEntryCollapsed` 全仓零调用）；②AC-07 `--text-3` on `--surface-2` 实算 4.23:1，落在 `.approvals-done` 上，而 PAIRS 漏了这一行；③AC-05 `#file-upload` 是 `.sr-only` 但可聚焦，焦点环被 clip 裁没；④AC-05 `.submit-bar textarea:focus` 压掉了全局 `:focus-visible` 的环。<br>**另修 3 处假绿门禁**：裸色值门禁有三个洞（三位简写/八位 alpha/同声明含 `var()` 就跳过）、焦点门禁遇 `@media` 错位、顶层 `resolveColor` 只跳一层导致"绿≠红"比的是令牌名字符串。<br>**真机已验**（4173 实测）：四视口 390/768/1280/1440 零横向溢出；200% 缩放（等效 640×360）无横滚且滚到底后主操作完整可见；Tab 序 11 元素无陷阱；`:focus-visible` 实渲 `2px solid`。<br>**剩余缺口 2 条**（不自欺）：NVDA 听感（AC-06 的非结构部分）、AC2-11 的单帧毫秒数 |

## 测试规模

逐轮增量表已删（见表头 §1.5 说明）：它记的是"某一轮加了哪些断言"，而那件事
提交信息里已经有了，文档里的副本只会漂。要看当前规模跑 `npx vitest run`。

当前（2026-08-08）**641 / 21 文件**，前五大：`ui-app` 134、`ui-a11y` 86、
`ui-faces` 60、`ui-patch` 60、`ui-server` 52。
