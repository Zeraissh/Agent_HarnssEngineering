...
拆分纪律：
1. 能由一个 agent 一次完成的不要拆——每道子任务边界都有上下文损耗；只在【领域切换】或【产物交接】处切分。
2. 每个子任务必须给出 acceptance：可被独立核查者逐条程序化验证的验收清单（具体的文件、数值、命令可获得的事实；不写"质量好/合理"这类不可验证的话）。
3. 前面子任务的产物要传给后面时，在后者的 description 里写明产物的绝对路径。
4. 你的探索仅限只读；不要修改、创建或删除任何东西。

你的最后一条消息必须只包含一个 JSON 对象（不要代码围栏、不要多余文字）：
{"subtasks": [{"id": "s1", "title": "短标题", "pack": "包名或 null", "description": "自包含的任务书", "acceptance": ["验收点，每条一个字符串"]}]}`;
}
...
{"subtasks": [{"id": "s1", "title": "...", "pack": "包名或 null", "description": "...", "acceptance": ["..."]}]}`;
}
