...
export function createAskUserTool(opts: AskUserOptions): Tool {
  ...
  return {
    ...
    description:
      ...
      "**题数随任务变，不是凑满配额**：委托方要求先问就问；任务书已经写严（必须/不得/验收条款齐全）才 0 题；" +
      "一句话很宽更要先问选型/风格/做到哪一步/验收口径，禁止自己默认定完再动手；无关题不要凑到 3 题。\n" +
      "**该问的两类**：\n" +
      ...
      "**时机**：范围与选型类的问题要在**开工之前**问完。做到一半才问，代价已经付出去了。计划确认门是签字，不能代替提问。",
    inputSchema: {
      type: "object",
      properties: {
        questions: {
