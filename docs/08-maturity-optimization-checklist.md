...
| 状态 | ID | 优化项 | I/R/E | 优先分 | 完成定义 |
|---|---|---|---:|---:|---|
| [~] | EVAL-01 | Held-out 真实任务集 | 5/5/4 | 20 | 建立 20–50 个不参与提示/实现调优的任务，覆盖编辑、调试、澄清、权限、恢复、MCP、多文件与失败场景。**仪器已落地（2026-09-02）**：`eval/cases-heldout.ts` 25 条 `ho-*`；`AB_SUITE=heldout|research|all`；nightly/release 六件套切到 held-out；与 research `cases.ts` id 互斥。**全量真实 provider 基线已跑（2026-09-03，v1.3.0）**：25 × 3 = 75 run、`deepseek-v4-flash` × `baseline`，**75/75**（Wilson95 [95.1%, 100%]），651k tokens / 501 s；`eval/baselines/heldout-v1.3.0.json` + `eval/heldout-report-v1.3.0.md`。零失败 taxonomy；transcript 回放抓出 Windows bash 子进程丢父 PATH 回归（已修，基线数字为修复前）。**仍开**：活 MCP HIL 面、调试类真机任务；`ho-workdir-escape-denied` 在 report 隔离模式下量的是模型自律而非圈禁 |
...
| EVAL-01 held-out | `eval/cases-heldout.ts`：**25** 条 `ho-*`（编辑/多文件/恢复/圈禁逃逸/成文口径/缺 MCP 旁路/条件分支/结构化抽取）；`eval/suite.ts` `resolveAbSuite`；`AB_SUITE` + nightly/release `AB_CASES` 切到 `HELDOUT_NIGHTLY_IDS` 六件套；`test/eval-heldout.test.ts` 锁规模/id 互斥/表面覆盖。纪律：本会话**未**为追分改 prompt/包。research `eval/cases.ts` **不是** held-out。**v1.3.0 全量基线（2026-09-03）**：detached worktree 钉 tag，`AB_REPS=3` × 25 = 75 run，`deepseek-v4-flash`/`baseline`，75/75，651,543 tokens（p50 5.1k / p95 29.2k 每 run），501 s（p50 4.2 s / p95 18.3 s）；六件套折算三夜 6/6、34.5k–54.9k tok、25–27 s（地板 150k/300s 余量 2.7×/11×）。产物 `eval/baselines/heldout-v1.3.0.json`、`eval/heldout-report-v1.3.0.md`；ab-log + 75 transcript 留本机 `D:\Work\scratch\heldout-v1.3.0-artifacts`。顺带修两缺陷：bash 子进程 Windows 下丢父 PATH（1653b7b 回归，`prependBashPath`）、`stats.ts` 不认 ab-log 的 `wallMs`（wall 列恒 "—"） | 无活 MCP/HIL 调试任务；n=3 单用例 Wilson 下界仅 43.8%，置信度靠 nightly 跨夜累积；`ho-workdir-escape-denied` 三次通过全靠模型自律（bash 在 report 模式无圈禁、write_file 圈禁未被触发）；地板未动，仅建议（见报告 §5） |
| E2E-01 Phase 1 | **2026-09-03**：`eval/e2e-web.ts` + Playwright Chromium 对 **`dist/ui/serve.js`** + mock-provider（零真实 token）。六场景：approval allow/deny（真点卡片）、SSE `Last-Event-ID` 续传无回放/无重复 seq、崩溃后 same-run 热恢复（含 Playwright「同运行热恢复」）、无 checkpoint 飞行崩溃 `sameRunResume=false` 诚实、**会话中心化**（核查过的轮次之后追加 200、正史 + 裁决摘要续跑、`judgedTurn`、本轮不核查则 run_end 不挂旧裁决）。CI job `e2e-web`（install chromium --with-deps）。容器：`scripts/container-health-smoke.mjs` 接进 `container` job（镜像 `/health`）；既有 OCI 逃逸 canary 继续作隔离证据。`npm run e2e:web` / `e2e:container` | **Electron 打包启动/升级/卸载未进 CI**：cross-app 仅有本机 `desktop:smoke`（win-unpacked `/health`），无 upgrade/uninstall 自动化——不在本会话发明 electron-builder E2E。Android=E2E-02。BASE-04 仍开（真实 provider / 已安装 Electron / HIL 未齐） |
...
| MEM-01 Phase C | 分级流水线，便宜的先上：① **入口截断** `snipToolResult`（ToolExecutor 边界，`AGENT_TOOL_RESULT_MAX_CHARS` 默认 40k > 内置 30k，MCP 返回无上限的兜底；标记带分页提示；事件与正史同一份）；② **tier 2** `collapseOldTurns`：tier 1 后按字符/4 估算仍在水位上或无可置换 → 保护窗外、任务首条之后的旧轮折叠为一个 `[compacted_turns]` 块（assistant 首句 / 工具调用 / 结果首行 / user 文本；配对永不拆散——窗首是 tool_result 时其 assistant 一并保留；只合并不二折 = 幂等；被折叠的小结果也过账本）；③ **反应式** `isContextOverflowError`（Anthropic「prompt is too long」/ OpenAI `context_length_exceeded`，两条真实 SDK 的形状经 mock 锁住）→ loop 每轮一次 `force + protectRecent=2` 硬压缩重发同一轮，事件 `compaction{reactive:true, collapsedTurns}`，不占瞬时重试额度；仍超长 → `finish("error")`，分类前缀 `context_overflow`。CLI + `app.js` 三处同提交；`eval/mock-provider` 新故障 `context_overflow` + 确定性场景 `context-overflow-reactive-compaction`（13/13）；mutation-smoke 新增 3 个变异（tier2 跳过 / 反应式跳过 / 截断绕过），16/16 killed。**真端点复核（2026-09-03，deepseek-v4-flash @ api.deepseek.com/anthropic）**：实测窗口 **1,048,576 tokens**（不是假定的 128k；16×30k 字符 = 178k tokens 一次请求被正常接受），端点按 messages + max_tokens 之和计超长；兼容路由回的是 **OpenAI 信封 + `code:"invalid_request_error"`**（不是 `context_length_exceeded`，SDK 包成 `BadRequestError`，message =「400 {…json…}」，content-type octet-stream），只靠 message 里「maximum context length」一句识别——此前两条 mock 形状都覆盖不到，已加逐字锁 + 变异验证。112 文件 / 14 页清单的 CLI 真跑（`AGENT_CONTEXT_LIMIT=1e7` 使水位路径永不触发）：第 12 轮请求 ≈ 987k + 64k 撞 400 → `compaction{reactive}` dropped 72 blocks / collapsed 10 turns / ledger 22 → 重发 107k tokens 成功 → `completed`（28 轮，in 1.38M / cacheR 10.6M / out 21.7k，352 s），summary 112/112 逐字正确；水位路径 0 次。**折叠质量**：被 tier 1 置换过的结果在折叠块里只剩「(elided)」，首行事实随之丢失，模型靠占位符提示自行 `read_file limit=1` 补读 72 次（8 轮）才凑齐——**已修（同日）**：占位符带 `excerpt:` 行（原文首个非空行 ≤100 字符 + 行数，`is_error` 取错误行），tier 2 折叠已置换的块时复用该摘录（只有旧占位符才退回「(elided)」）；台账行新增 `compaction { proactive, reactive, droppedBlocks, collapsedTurns }`，`npm run ledger` 一行摘要；mutation-smoke +3（摘录丢弃 / 折叠写回 elided / 台账不计数） | **仍不压缩的**：保护窗内（默认最近 6 条 / 反应式 2 条）的一切；任务首条 user 消息（含 `<context>` 块，可能很大）；`[compact_ledger]` 本身（有 20 条/桶上限但不缩）；折叠块只按行摘要不做语义合并，轮数多时块本身会长（每轮 1~2 行）；tier 2 的触发用 4 字符/token 粗估，不读真实 token；system prompt / tools 声明不在压缩范围；反应式只重发一次，第二次超长即 error（不做"再砍保护窗"）；未开 Phase B 时折叠块与账本全是启发式，长 assistant 推理里的非模板事实会随折叠丢失 |
