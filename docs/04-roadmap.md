...
- [x] planner 依赖图产出质量（同报告 + REPS=5 复现批）：**拆分率 5/9（~50/50 不稳定，
  初批"1/4 倾向不拆"未复现——小样本噪声）**；九份计划零非法图（悬空/成环率 0，
  fail-closed 零触发）——瓶颈是"拆不拆"的摇摆，不是"写不写得对图"。
  planner 产的 fan-out 计划进并行调度器 4/4 精确贴关键路径（泛化成立）。
  附加发现：planner 版汇总步比手写注入版慢 3–4×（缺"只消费上游产物"收口约束）——
  任务书一句收口 = 下游一半墙钟。
  采纳建议（数据已足）：①planned 拆分后默认并行（串行默认让一半 run 白付拆分成本）；
  ②planner 提示词补汇总收口一句
- [x] 两条采纳已实施（9508333）：concurrency:"auto"=min(3,层宽)，CLI --plan 默认 auto
  （--parallel=N 覆盖、=1 退串行）；planner 拆分纪律第 5 条收口句。121 单测
- [x] 收益曲线（par-fanout6，6 分片 × par1/3/6 × 3，9/9 pass）：serial 210s →
  par3 93s（−56%）→ par6 79s（−62%，对 par3 仅 −15%）；par6 仍精确贴关键路径，
  衰减来自汇总串行分量（Amdahl）+ 并发 12 路流的端点慢尾（44–53s，零失败）——
