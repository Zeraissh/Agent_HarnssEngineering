# 真实任务案例 #7 — Web 宿主 UI × 委托方整改报告闭环（2026-08-05 ~ 08-06，第六节回望补于 08-08）
...
（s3a~s3e）→ 依赖安全收尾。单测 146 → **225**（这两个数字是 v1 收口那一刻的快照，
不是当前值；后续演进见文末「后续」与第六节）。
...
> ⧗ 这一节记的是 **v1 收口那一刻**。此后界面整体重做过一次（v2），
> 「全部关闭」这个结论本身在 2026-08-08 的 AC2-18 复验里被推翻过三条——见第六节。

---

## 后续（2026-08-06 ~ 08-08）

本案正文停在 v1 整改收口。**故事没有到此为止**，但后续没有并进正文——
它们各自有更合适的载体，重复叙述只会造成两份会漂移的记录。

（本表刻意**不写单测总数**：那类写死的计数必然过期，本项目已经因此产生过三处
假数字。要看当前规模跑一次 `npx vitest run`。见 findings 47。）

| 阶段 | 载体 |
|---|---|
| **UI v2**（R0–R11，34 个问题项 V-01~V-34） | [`ui/remediation-v2.md`](../../ui/remediation-v2.md)（任务书源）+ [`ui/remediation-v2-status.md`](../../ui/remediation-v2-status.md)（逐条关闭状态与第一手证据） |
| **v2 之后的未做项与逐条理由** | [`docs/06-backlog.md`](../06-backlog.md)（交接文档，含"为什么不那么做"） |
| **backlog 开工两批**（schema 校验 / 抖动 / 文本流式 / 计划确认门 / MCP 接入 / 核查预算 / 收口续跑 / 段级续跑 / 思考透出 / Markdown 渲染） | `docs/06-backlog.md` 各节的"已实施"标注 |
| **委托方三轮界面反馈**（待办无限堆叠 → 钉底坞；输入框移到底部；表格真渲染；追加框与提交栏合并） | `docs/05-findings.md` 发现 42~45 |
| **AC2-18 逐条复验 + §1.5 状态表重排** | `ui/remediation-v2-status.md` 的 AC2-18 行 |
...
## 六、回望：这个案子最终讲的是什么（2026-08-08）

本案是全部案例里**唯一被反复重做的那个**——同一套界面经历了
建设（s1/s2）→ 委托方报告整改（s3a~s3e）→ 整体重做（v2 的 R0~R11）→
backlog 两批 → 三轮界面反馈 → AC2-18 逐条复验。正因如此，它照出了一件
只有"活得够久的软件"才照得出的事：

> **一条验收被"关闭"之后，会在无人察觉时失效——因为承载它的结构被重做了，
> 而看守它的断言还在绿着。**

三条实证（全部来自 AC2-18 复验，2026-08-08）：

- **R-03「结果不被日志埋掉」**：v1 靠"三标签"承载，v2 换成"四决定因素卡 + 下钻面"。
  意图仍在，但**把结果卡挪到下钻面之后，当时四个 UI 测试文件（约 330 条）
  无一变红**——`.outcome-card` 在整个 `test/` 下零命中，取代物上线时没人给它配锁，
  这条验收实际上已经裸奔了两个大版本。（补锁之后同一变异当场变红，已复验。）
- **AC-04「两次操作可达原始详情」**：宿主自己另写了一套折叠 toggle，
  被测的那个纯函数**全仓零调用**。测试一直绿，测的是产品不用的那份实现；
  真实行为是第一次点击完全没有反馈。
- **AC-05「焦点可见」**：全局 `:focus-visible` 规则一直在、断言一直绿，
  但组件层后加的 `.submit-bar textarea:focus` 特指度更高，把环压掉了。
  只有真机 Tab 一次才看得见。

这三条的形态是同一个：**断言在、绿着，但它保护的东西已经不在被测范围内。**
所以本案给出的做法，不是"更多测试"，而是两条更硬的纪律：

1. **复验一条验收要用变异测试**——把它声称保护的东西改坏，看有没有测试变红。
   逐条打勾几乎必然签出假通过（findings 46）。
2. **重做一处结构时，同一个提交里把旧验收的锁迁移过去**。取代物没被守住，
   等于悄悄丢了一条验收——这是三类里最危险的一类，因为状态表上它还写着 ✅。

与案例 #5「落盘矩阵实验」、案例 #8「三层归因顺序」并列，构成本项目关于
**"绿色不等于对"** 的三个不同侧面：#5 是声明层绿 ≠ 运行时对，
#8 是跑得久 ≠ 判得准，#7 是**曾经验过 ≠ 现在还在被验**。
