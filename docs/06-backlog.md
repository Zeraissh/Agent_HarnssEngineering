...
- **借鉴清单 `docs/09-借鉴清单.md`（2026-09-03）** — 外来机制进 docs/08 编号队列前的**准入闸门**：12 个机制的
  借 / 变形借 / 不借结论 + 五条借鉴规矩（含四条不得稀释的不变量）+ 八条待借条目（判据阈值先写，全部默认关）。
  **接手要点**：D3（deny 档 + manual/plan/auto 三档）是第一顺位且是 hooks 与分类器的前置；分类器有硬前置
