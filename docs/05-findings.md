...
真实案例构成一条从"需求 → 执行 → 暴露问题 → 方法论/工具改进 → 回灌 harness →
下一案例受益"的闭环飞轮。每案产出的改进已在后续案例中验证生效。

（此处原写"五个案例"，而下表长到 #8 之后那句话就成了假的——**凡在标题里写数量，
就得在加一行时同步改**，本项目已经因这类写死的计数出过几次错，见发现 47。
所以标题改为不带数量。）
...
| **#6** | 把五案经验提炼成本研究总结文档（harness 给自己写论文），并**故意保留一条主观验收**当探针 | ① 主观验收（"提炼而非罗列"）下 programmatic verifier 的行为未知；② join 段 verifier 无白名单，本该重演案例 #4 的 fail-closed 空转 | ① **"判不动就自陈判法"一句纪律换来结构化部分裁决**——能判的判、判不了的逐条移交人工（杠杆定律的又一实证）；② 由此提取出 **rubric-verifier 的三值裁决形态**（可判/不可判/主观），即今天的 passed·unverified·advisory | `docs/cases/case-06-findings-doc.md`：5/5 全过，并行省 44%；本文件即产物 |
| **#7** | Web 宿主 UI：CLI → 浏览器实时控制台，并按委托方正式整改报告闭环 | ① 静态断言只守护"写了什么"（孤儿 `role=option`）；② **一条验收被"关闭"后会在结构重做时悄悄失效**——AC2-18 复验用变异测试照出三条；③ 宿主学："harness 有、宿主没接"成为稳定规律 | ① **委托方正式报告当验收基线**（不二次转译）；② **变异测试作为复验手段**（发现 46）；③ **axe-core 常驻门禁**；④ **门禁上线要先证明它会红**；⑤ **观测装置也会骗人**（发现 49）；⑥ **便利功能不许削掉它所在的不变量**（发现 50） | `docs/cases/case-07-web-host-ui.md`：R-01~R-08 + AC-01~AC-10 全关闭；AC2-18 复验补修 4 处实缺陷 + 3 处假绿门禁 |
| **#8** | STM32L1 CRC 外设自检固件，跨 coding→debug 两域交付（计划确认门首秀） | ① 芯片系列相关常量是 agent 的稳定盲区（`1u<<6` 是 F1 的 CRCEN，L1 在 bit 12）；② 核查失败被误归因为"预算不够"，实为"被成文规则挡住" | ① **核查失败的三层归因顺序**：先怀疑规则 → 再怀疑预算 → 最后才怀疑能力（发现 37~41）；② `DomainPack.verify.maxTurns` 按域可覆盖；③ 段级瞬时失败带正史续跑 | `docs/cases/case-08-crc-selfcheck-hil.md`：同一产物四跑 A/B/C/D，ELF SHA256 逐字节相同 |
...
**发现 48 —— 量之前先别猜瓶颈：这次量出来的和预期完全相反**
（AC2-11 单帧量化，2026-08-08）

「1000 事件单帧 < 16ms」这条验收挂了很久没量。我下手前的假设是：`appendOnly`
每次都遍历全部条目（它的注释还宣称"单次代价是 O(新增条数)"，那句是错的），
所以瓶颈在脚本。**量完发现全错**：

| 组件 | 2000 条时的耗时 |
|---|---|
| `reduceEvents` | 0.8 ms |
| `deriveLogEntries` | 0.8 ms |
| `appendOnly` 全量走一遍 | **0.3 ms** |
| 整帧（Loop 面，含日志） | 24.4 ms |
| 整帧（Context 面，无日志） | **0.9 ms** |

最后两行是判据：**同一份状态、同一次渲染，只是不画日志，单帧就从 24.4 掉到 0.9**。
钱全花在一棵 1.3 万节点日志树的**布局**上，脚本加起来不到 2ms。

修法因此不是优化 JS，而是**让浏览器别去排视口外的版**：一行
`content-visibility: auto`。实测 11.2→3.5（n=1000）、21.7→8.6（n=2000）、
46.5→15.2（n=4000），预算线从 ~1400 条推到 ~4200 条。零 JS、零行为变化，
且跳过渲染的条目**仍在可访问性树里**、仍能被页内查找命中（这三条都真机确认过——
换成 `content-visibility: hidden` 就会破坏它们，所以回归锁里专门禁了 hidden）。

> **两条**：
> ① **先量再改**。我这次的假设指向了一个 0.3ms 的东西；照着假设去优化，
>    会花很大力气重写 `appendOnly` 而单帧一毫秒都省不下来。
> ② **注释里的复杂度声明也要被证伪**。`appendOnly` 那句 "O(新增条数)" 是假的
>    （它确实遍历全部），只是恰好不重要——**假的但不重要**和**真的**是两回事，
>    下一个人会照着那句话做判断。

**发现 49 —— 真机结论要先问：这是被测对象的性质，还是我这套观测装置的性质？**
（同轮两次踩到，2026-08-08）

"真机实测"这四个字本身也会骗人。一轮界面改造里我**两次**把观测环境的性质
当成了产品缺陷，第一次还写进了代码注释、当成实证提交：

| 我观测到 | 我当时的结论 | 真相 |
|---|---|---|
| 点"回到底部"按钮，`scrollTop` 一动不动 | "`scrollTo({behavior:'smooth'})` 对本容器是**静默空操作**" | 自动化面板**不合成画面**，任何带动画的滚动都不会推进。把 `scroll-behavior` 临时置成 `auto`，同一句立刻落到位 |
| 250ms 的采样只跑了 ~1s 一次，还有一次 7s 空档 | "流式期间**主线程被长时间阻塞**" | 面板**节流后台定时器**。`PerformanceObserver` 观测 longtask 录到 **0 条**——主线程根本没堵 |

两次的形态一模一样：**观测装置在被测对象与我之间加了一层，而我把那一层的
行为算到了被测对象头上。** 第一次尤其危险——我不只是判断错了，还把"真机实测"
四个字写进注释当作证据，让下一个人没法再怀疑它。

> **纪律**：真机测量前先问一句「这套装置本身会不会改变我要测的量」，
> 并**优先选不受装置影响的判据**。
> 第二例的正解就是这么找到的：与其数"我的定时器多久跑一次"（受节流影响），
> 不如问浏览器"有没有长任务"（`PerformanceObserver`，不受影响）。
> 第一例同理：与其量"动画后落到哪"，不如**先关掉动画再量落点**——
> 动效和落点是两件事，只有后者是我要验的。

这条与发现 46（变异测试）互补：那条防的是"断言没在守护它声称守护的东西"，
这条防的是"观测到的现象不是被测对象产生的"。**两者都是在问同一个问题：
我这个绿色/红色，到底是谁给的。**

---

**发现 50 —— 把便利做成规则时，省掉的必须只是点击，不是记录**
（常驻放行 + 停止按钮，2026-08-08）

委托方提了两个纯体验诉求："每次审批都要人手点击很麻烦"、"要有停止按钮"。
两个都做了，而两个都在实现时踩到同一类线：**便利功能很容易顺手把它所在的
那套不变量一起削掉**。

**① 常驻放行**（2026-08-27 已收紧为本次对话内“同一工具 + 相同参数”）。
最初版本只绑定工具名；安全复核证明这会让第二条完全不同的 bash command、路径或
设备参数复用旧批准，因此不能作为成熟权限边界。现在三条边界缺任何一条，
它就从"省事"变成"把审批门拆了"：

- **逐 run**——active grant 绑定 `boundRunId`；完整 main checkpoint 只落版本化
  **审计快照**，重启/归档派生的新 run 必须从零开始问并记录 `not_inherited`；
- **逐工具契约元数据 + 精确输入**——除工具名和递归规范化输入的 SHA-256 外，还绑定
  description/schema/权限/并发标记/复用策略 fingerprint（不声称覆盖 execute/build
  实现）；对象 key 顺序不同可复用，command/path/device
  任一值变化或工具定义变化都必须重新审批；
- **权限有上限**——未声明策略的 ask 工具缺省 `once`；只有工具显式允许时才能建立
  exact-input grant，并同时受绝对 TTL 与最大自动复用次数约束。`bash`、`write_file`、
  `describe_image` 和 ask 型 MCP 保持单次审批；`describe_image` 即使 path 相同也可能
  已换成另一份文件，在资源内容摘要进入授权主体前不能复用。当前仅 `fetch_url` 显式
  开放受限 exact-input；
- **自动放行照样进事件流**，标 `actor: "auto-rule"`、`inputScope: "exact-input"`
  和 `inputHash`；过期、次数耗尽、工具变化、时钟回拨与跨 run 不继承在触发检查时
  也各有事件。

第三条最要紧。前两条是范围，第三条是**可审计性**：一条分不清"人点的"
还是"规则放的"的记录，比多点几下危险得多——事后复盘时你会以为每一次写入
都有人看过。**省掉的是点击，不是记录。**
同理，规则一旦存在就必须显示在界面上，并显示工具名、hash 摘要、剩余次数与期限；
历史快照必须与 active grant 分栏：**悄悄不再问你的审批门，
和没有审批门是一回事。**

**② 停止按钮**。`AgentLoop.run(input, signal)` 从第一天就收 `AbortSignal`——
但编排层与 Web 宿主从没往下传（findings 34 那条规律的第九例）。接上之后
**第一次真机测试就失败了**：按下停止 8 秒，run 仍在 running。
原因是循环只在**每轮模型调用之前**查中止位，而一次长生成就是一次调用；
signal 没传给 SDK，在飞的 HTTP 请求掐不掉。

> **一个不能停的停止按钮比没有更糟**：没有按钮时人知道自己只能等，
> 有一个按不动的按钮时人以为已经停了。

补法是把 signal 一路透到 `ModelClient.send(req, onDelta, signal)`（两个客户端
都改）。修完再测：running → done 用时 4 秒。

还有半条纪律是从 V-04 借来的：中止**必须与 error 分开**（新增
`stopReason: "aborted"`）。混进 error 有两个后果——界面把"我叫停的"画成
"它崩了"；而 error 路径会触发段级续跑（9.8），于是"停止"变成
"停一下又自己接着跑"。**人的决定不是系统的故障，这两类在状态机里就得分开。**
