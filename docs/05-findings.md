...
| **#7** | Web 宿主 UI：CLI → 浏览器实时控制台，并按委托方正式整改报告闭环 | ① 静态断言只守护"写了什么"（孤儿 `role=option`）；② **一条验收被"关闭"后会在结构重做时悄悄失效**——AC2-18 复验用变异测试照出三条；③ 宿主学："harness 有、宿主没接"成为稳定规律 | ① **委托方正式报告当验收基线**（不二次转译）；② **变异测试作为复验手段**（发现 46）；③ **axe-core 常驻门禁**；④ **门禁上线要先证明它会红** | `docs/cases/case-07-web-host-ui.md`：R-01~R-08 + AC-01~AC-10 全关闭；AC2-18 复验补修 4 处实缺陷 + 3 处假绿门禁 |
